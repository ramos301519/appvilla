...
class Subcategoria_model extends CI_Model {
    ...
    var $tablectg = 'tbcategoria'; 
    var $table = 'tbsubcategoria';
    var $colum = array('categoria','nombre','descripcion');
    var $order = array('nombre' => 'desc');

    private function _get_datatables_query() {
        $this->db->select('a.id,b.nombre as categoria,a.nombre,a.descripcion');
        $this->db->from($this->table.' a');
        $this->db->join($this->tablectg . ' b', 'a.categoria_id=b.id');        
    ...
    public function get_by_all($idctg) {
        $this->db->select('id,nombre');
        $this->db->from($this->table);
        $this->db->where('empresa_id', $this->session->userdata('ciaid'));
        $this->db->where('categoria_id',$idctg);
        $this->db->where('enabled',1);
        $query = $this->db->get();
        return $query->result();
    }
