var save_methodp; //for save method string
var tablep;
$(function() {
    tablep = $('#tablap').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": "Almacen/Productos_control/ajax_list",
            "type": "POST"
        },
        "columnDefs": [{
            "targets": [-1],
...
function edit_p(id) {
    ...
    $.ajax({
        url: "Almacen/Productos_control/ajax_edit/" + id,
        type: "GET",
        dataType: "JSON",
        beforeSend: function() {
            loadshowmodal();
        ...
        success: function(data) {
            ...
            if (data.status === true) {
                ...
                $('#modal_formp').modal('show');
                $('#modal_formp .modal-title').text('Editar Producto');
            } else {
                Swal.fire({
                    icon: 'error',
...
function savep() {
    var url;
    if (save_methodp == 'add') {
        url = "Almacen/Productos_control/ajax_add";
    } else {
        url = "Almacen/Productos_control/ajax_update";
    }
...
function delete_p(id) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "Almacen/Productos_control/ajax_delete",
                type: "POST",
                data: { id: id },
                dataType: "JSON",
                beforeSend: function() {
